...
public class Team {
    ...
    var x :Double = 1000.0 , y : Double = 1000.0
    var x_axis : Double = 0.0 , y_axis : Double = 0.0
    var radius : Double , member : Int
  ...
  public init(color :  String, team_name : String, mem : Int, play : Array<Player>){
        ...
        self.member = mem
        self.player = play
        self.radius = Double(10 * mem)
        self.lose = false
        self.winner = true
        self.x_axis = self.location_x_axis()
        self.y_axis = self.location_y_axis()
   ...
   public func setRadius(sRadius  : Double) -> Void {
        self.radius = sRadius
    }
    
   public func getX_axis() -> Double {
        return x_axis
    }
  public func setX_axis(sX_axis : Double) ->  Void{
        self.x_axis = sX_axis
    }
    
   public func getY_axis() -> Double {
        return y_axis
    }
   public func setY_axis(sY_axis : Double) ->  Void{
        self.y_axis = sY_axis
    }
   public func location_x_axis() -> Double {
        let n = Double.random(in: 1 ... x)
        return n
    }
   public func location_y_axis() -> Double {
        let n = Double.random(in: 1 ... y)
        return n
    }
    
   public func moveTeam() -> Void {
        if self.getX_axis() > 20  {
            self.setX_axis(sX_axis: self.getX_axis() - Double.random(in: 1 ... 50))
        } else {
            setX_axis(sX_axis: 0)
        }
        
        if self.getY_axis() > 20  {
            self.setY_axis(sY_axis: self.getX_axis() - Double.random(in: 1 ... 50))
        } else {
            setY_axis(sY_axis: 0)
        }
    }
